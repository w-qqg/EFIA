...
cls_ft_channel = 256
sa_dim = 256
model_name = "EbdSC_MHFI_SC_onFlickr_LR{}".format(str(lr))
# model_name = "Map-Red-MHSA={}d_lr={}".format(str(sa_dim),str(lr))
logger = initLogger(model_name)

logger.info("Freeze Convs, Train Map, Embed(SC), MultiheadInteraction(SC) and FC")
...
class MultiheadInteraction(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = self.act2(self.split_heads(q, batch_size))  # (batch_size, seq_len_q, num_heads, depth_q) (m,l,h,d)
        k = self.act1(self.split_heads(k, batch_size))  # (batch_size,  seq_len_k, num_heads, depth_q) (m,j,h,d)
...
class InnerAttention(nn.Module):
    ...
    def __init__(self, embed_dim=256, sa_dim=256, num_heads=8):
        ...
        self.mha_layer = MultiheadInteraction(d_model=sa_dim, num_heads=num_heads)
    ...
    def forward(self, x):
        ...
        k = self.K_layer(x)
        v = self.V_layer(x)
        out = self.mha_layer(q,k,v)
        out = self.dropout(out)
        if self.sa_dim == self.embed_dim :
            out += x
        else:
            out += self.scReduce(x)
        # print(out.shape)
        # out = out + self.scReduce(x)
        # x = out.permute([1,0,2])
        return out
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x_short_connection = [x_reduced_list[i] + x_enhanced_list[i] for i in range(8)]
        # ↑ 512*300 = bs*cls_ft_channel
        
        y_ = torch.stack(x_short_connection, 1) # 512,8,300 = bs, cls_num, feat_dim

        y = self.layer_MHA(y_)
        # print("layer_MHA out:", y.shape)
        y = y.flatten(1)
        # print("flatten out:", y.shape)
        y = self.fc(y)
...
if first_train:
    ...
    latest_model_path ='./ckpt/EbdWt_SC_MHSA_onTwitter_LR0.001_Ep5_test0.85394.pth'
