...
hyper_a = 0.85
lr = 1e-3
first_train = False # False True
opt = 'Adamax' # Adamax SGD
cls_ft_channel = hyper_embdim
sa_dim = hyper_embdim
model_name = "L1loss{}_Head{}_HasEb_PTSA_Flk_LR{}".format(str(hyper_a),str(hyper_head),str(lr))
# model_name = "Map-Red-MHSA={}d_lr={}".format(str(sa_dim),str(lr))
logger = initLogger(model_name)
logger.info("ReduceBranch use MaxPool, noEmbed, pytorch MHSA")
logger.info("NOFreeze ConvLayer, Train map, reduce, embed_weight(SC), MultiheadInteraction(no SC)and FC")
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.2, bias=True, add_bias_kv=False, add_zero_attn=False):
    ...
    def _fun_mean(self, y ,fun):
        y_v = y.view(-1, y.size(2))
        act_value = fun(y_v)
        dim_ext = torch.sum(act_value, -1)
        dim_ext = dim_ext.reshape(-1, 1)
        res = act_value / dim_ext
        return res.view(y.shape)
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        attn_output_weights = torch.bmm(q, k.transpose(1, 2)) + 1

        # ~~~ HERE ~~~
        # attn_output_weights = torch.tanh(attn_output_weights) + 1

        assert list(attn_output_weights.size()) == [bsz * self.num_heads, tgt_len, src_len]

        # ~~~ HERE ~~~
        # attn_output_weights = self._fun_mean(attn_output_weights, self._doNothing)
...
class FeatEmbedFusion(nn.Module):
    def __init__(self, feat_channel, embed_dim):
        ...
        d_sum = feat_channel+embed_dim
        self.fc1 = nn.Linear(d_sum, int(d_sum*1.5))
        self.fc2 = nn.Linear(int(d_sum*1.5), feat_channel)
        self.relu = nn.ReLU()
        self.sf = nn.Softmax(dim=-1)
    def forward(self, feat, embed):
        embed = embed.unsqueeze(0).repeat(feat.size(0),1)
        both = torch.cat([feat,embed], dim=1)
        x = self.relu(self.fc1(both))
...
class InnerAttention(nn.Module):
    ...
    def __init__(self, embed_dim=256, sa_dim=256, num_heads=8):
        ...
        # self.mha_layer = MultiheadAttention(embed_dim=sa_dim, num_heads=num_heads)
    ...
    def forward(self, x):
        ...
        x = self.dropout(out)
        x = (out.permute([1,0,2]) + x_id)/2
        # print("InnerAttention out:",x.shape)
        return x

class Map2ClsBranch(nn.Module):

    def __init__(self, embed_dim=256, sa_dim=256, num_heads=8):
        super(Map2ClsBranch, self).__init__()
        self.conv = nn.Conv2d(2048, cls_ft_channel, 3)
        self.bn = nn.BatchNorm2d(cls_ft_channel)
        self.relu = nn.ReLU(inplace=True)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, embeds, num_classes=8, zero_init_residual=False,
                 ...
                 norm_layer=None, cls_ft_channel=256, sa_dim=256, hyper_head=8):
        ...
        self.layer_map = nn.ModuleList([Map2ClsBranch() for i in range(8)])
        self.layer_reduce = nn.ModuleList([BranchReduce(self.cls_ft_channel) for i in range(8)])
        self.layer_MHA = InnerAttention(embed_dim=self.cls_ft_channel, sa_dim=self.sa_dim, num_heads=hyper_head)
    ...
    def _forward_impl(self, x):
        ...
        # y = torch.stack(x_reduced_list, 1) # 512,8,300 = bs, cls_num, feat_dim
        ...
        y = self.sigmoid(y)
        # x = self.output(y)
        entropy_estimate = torch.std(y, dim=1)

        return y, entropy_estimate
...
cls_ft_channel=cls_ft_channel, sa_dim=sa_dim, embeds = embeds_Flickr.cuda(), hyper_head = hyper_head).cuda()
# print(model)
...
if first_train:
    ...
    # ckpt_path ='./ckpt/Base_Twitter_lr_0.0001_Ep29_test0.39578.pth'
    ...
    # latest_model_path ='./ckpt/EbSc_TFelu_Flkr_LR0.001_latest.pth'
    latest_model_path ='./ckpt/L1loss0.85_HasEb_PTSA_Flk_LR0.001_Ep18_test0.85512.pth'
...
visual_dataset = LDLdataset('res_visual.txt',mode="test", root=root,  vote_num = vote_num)


train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=bs, shuffle=True, num_workers=0)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=int(bs*1.5), shuffle=False, num_workers=0)
visual_dataset = torch.utils.data.DataLoader(visual_dataset, batch_size=1, shuffle=False, num_workers=0)

len(train_dataset),len(test_dataset),len(visual_dataset)
...
#     for data, cls, fn in eval_loader:
        data = data.to(device)
        optimizer.zero_grad()
        output, density = model(data)
        label = ground.to(device)
        # l1 = L1Std((output),(label)) L1与EMD两种loss没有相差很悬殊(3跟2)
        # Loss_Function
        loss = EMD((output),(label))# + 0.8*KL_bm((output),(label))
        loss_reg = L1loss(density, torch.std(label, dim=1))
        loss_mul = hyper_a*loss + (1-hyper_a)*loss_reg

        loss_mul.backward()

        loss_all += loss_mul.item()
        optimizer.step()

        if num_data==1 or num_data%50 == 0:
...
# %%
# 提前测试网络
test_acc, test_COS, test_KL = evaluate(visual_dataset,"visual_dataset")
...
# %%
# 开始训练
for e in range(epoch_start, 45):
    epoch_start += 1
    logger.info(" %s epoch started:", str(epoch_start))
    loss = train()
    if loss!= loss :
        logger.info("NaN Loss, Earlystopping ...")
        print("【NaN】")
        break
    ...
    print('Epoch: {:03d}, Loss: {:.5f} , Test Auc: {:.5f}, \nTest COS: {:.5f}, Test KL: {:.5f}'.
          format(e, loss, test_acc, test_COS, test_KL))
